...
<p align="center">
  <img src="docs/home.png" alt="ホーム画面。タイトル、カメラプレビュー、土下座する床の選択" width="420" />
</p>
...
マイクの ON/OFF と、計測画面のデモ用スライダーは **デバッグ用実験室** に入れています。本番の初画面は床選びと開始ボタンが主役で、カメラ切り替えはそのまま出します。

| デバッグ用実験室の出し方 | 動き |
| --- | --- |
| `npm run dev` | 最初から開く |
| タイトルを 5 回タップ | 開閉。ホームでも結果画面でも可 |
| `?debug=1` / `?debug=0` | URL で開く / 閉じる |

一度切り替えると、そのタブでは計測に進んでも状態を維持します。
